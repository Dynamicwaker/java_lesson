 ...
 * @version Created on 2019/3/25.10:48
 * @description 读写锁和ReentrantLock的比较
 */
...
public class TestReadAndWriteLock {
    ...
    private static class TestReadThread extends Thread {
        ...
        public void run() {
            try {
                TestReadAndWriteLock.handleRead(lock);
                //TestReadAndWriteLock.handleRead(readLock);
    ...
    private static class TestWriteThread extends Thread {
        ...
        public void run() {
            try {
                TestReadAndWriteLock.handleWrite(lock,new Random().nextInt(100));
                //TestReadAndWriteLock.handleWrite(writeLock, new Random().nextInt(100));
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
